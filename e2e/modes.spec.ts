...
test('comparison editors keep accessibility relationships unique', async ({ page }) => {
	await gotoApp(page);
	await switcher(page).getByRole('button', { name: 'Compare' }).click();
...
test('mode switcher carries the teaching tooltips', async ({ page }) => {
	...
	await page.getByRole('button', { name: 'Learn' }).click();
	await expect(page.locator('.coach')).toContainText('Select connected pixels of the same color.');
});
