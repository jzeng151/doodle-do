<script lang="ts">
	...
	const fork = $derived((session.comparisonVersion, session.comparisonFork as Doc));
	const forkCompositor = $derived(new Compositor(fork));
	...
	const forkScale = $derived(previewScale(fork));
	const forkWidth = $derived(fork.meta.width * forkScale);
	const forkHeight = $derived(fork.meta.height * forkScale);

	function previewScale(doc: Doc): number {
...
<section class="compare-mode" aria-labelledby="compare-heading">
	<header class="compare-header">
		<div>
			...
			<p>The fork stays unchanged while you edit the current animation.</p>
		</div>
		<div class="controls">
			...
			<button
				onclick={() => session.refreshComparisonFork()}
				title="Replace the saved fork with the current frames"
			>
				Refresh fork
			</button>
		</div>
	...
	<div class="split">
		...
		<figure>
			<figcaption>
				<strong>Fork</strong>
				<span>{forkFrame + 1} / {fork.frames.length} · Saved reference</span>
			</figcaption>
			<div class="viewport">
				<canvas
					bind:this={forkEl}
					class="compare-canvas"
					class:background-hidden={!session.showPreviewBackground}
					aria-label={`Fork animation, frame ${forkFrame + 1} of ${fork.frames.length}`}
					width={forkWidth}
					height={forkHeight}
				></canvas>
			</div>
